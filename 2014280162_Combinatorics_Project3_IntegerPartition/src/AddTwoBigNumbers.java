public class AddTwoBigNumbers {
	public static String addIntegers(String number1, String number2)
			throws Exception {
		...
		for (int i = num1char.length - 1; i >= 0; i--) {
			final int sum = Character.getNumericValue(num1char[i])
					+ Character.getNumericValue(num2char[i])
					+ Character.getNumericValue(carrry);
		...
		return String.valueOf(addition).trim();
	}

	// private static void validate(String number1, String number2)
	// throws Exception {
	// if (number1.trim().isEmpty() || number2.trim().isEmpty()
	// || !isNumber(number1) || !isNumber(number2)) {
